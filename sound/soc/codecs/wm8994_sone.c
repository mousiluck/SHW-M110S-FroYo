...
#define TUNING_VOIP_RCV_OPGAL_VOL               0x3B // 20h // [DJ22-2243] VoIP 0x34->0x39->0x3B (+2dB)
#define TUNING_VOIP_RCV_OPGAR_VOL               0x3B // 21h // [DJ22-2243] VoIP 0x34->0x39->0x3B (+2dB)
...
#define TUNING_VOIP_EAR_OUTPUTL_VOL             0x38 // 1Ch // [DJ25-1922] VoIP 0x38->0x35->0x38 (-1dB)
#define TUNING_VOIP_EAR_OUTPUTR_VOL             0x38 // 1Dh // [DJ25-1922] VoIP 0x38->0x35->0x38 (-1dB)
...
// VoIP Call
#define TUNING_VOIP_SPKL_VOL                    0x3B // 26h // [DJ22-2243] 0x3C->0x39->0x3B (+2dB)
...
// Main
// Normal Recording
#define TUNING_RECORD_MAIN_INPUTMIX_VOL         0x19 // 18h // [DE15-1659] 0x15->0x1F->0x0D->0x18->0x19 // H/W Req.
...
#define TUNING_VOIP_SPK_INPUTMIX_VOL            0x19 // 18h // [DJ22-2243] VoIP 0x1A->0x19(+21.0dB)
#define TUNING_VOIP_3P_INPUTMIX_VOL             0x19 // 18h // [DJ22-2243] VoIP 0x1A->0x19(+21.0dB)
	...
	switch(rec_path)
	{
		case MAIN:
			...
			if (wm8994->codec_state & (CALL_ACTIVE))
			{
				...
				{
					printk("[WM8994] turn off hpf in voip main mic...\n");
		...
		case EAR:
			...
			if (wm8994->codec_state & (CALL_ACTIVE))
			{
				...
				if(wm8994->codec_state & (VOIP_CALL_ACTIVE))
				{
					printk("[WM8994] turn off hpf in voip ear mic...\n");
...
//#ifdef FEATURE_VOIP_DRC
	...
	val &= ~(WM8994_AIF1ADC1L_HPF|WM8994_AIF1ADC1R_HPF);
	val |= (WM8994_AIF1ADC1L_HPF|WM8994_AIF1ADC1R_HPF); // hi-path filter on (L/R)
	wm8994_write(codec, WM8994_AIF1_ADC1_FILTERS, 0x0000); // [DJ22-2243] // 0x5800 -> 0x3800 -> 0x0000
#endif
