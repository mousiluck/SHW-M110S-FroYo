...
{
	if(gpio_get_value(GPIO_DET_35) || prox_factorymode)
	{
		proximity_onoff(1);
	}
	else
	{
		printk(KERN_DEBUG "GP2A Proximity GPIO_PS_ON = 0 because JACK_INT\n");
	       printk(KERN_DEBUG "[PROX_SENSOR] proximity_open, Factory Mode, Proximity Sensor Mode = %d \n",prox_factorymode);		
	}
