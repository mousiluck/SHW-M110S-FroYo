...
#if 1	//S1_KOR froyo [division of camera fw]
#define CE147_FW_F2_SE_PATH	"/system/firmware/SE/CE147F02.bin"
#define CE147_FW_F2_OP_PATH	"/system/firmware/OP/CE147F02.bin"
#else
...
static void ce147_ldo_en(bool onoff)
{
	...
	if(err) {
		printk(KERN_DEBUG "failed to request GPB7 for camera control\n");
		return;
	}
...
{	
	...
	printk(KERN_DEBUG "ce147_power_on by ce147 driver \n");
	
	/* CAM_MEGA_EN - GPJ0(6) */
	err = gpio_request(GPIO_CAM_MEGA_EN, "GPJ0");

	if(err) {
		printk(KERN_DEBUG "failed to request GPJ0(6) for camera control\n");
	...
	if(err) {
		printk(KERN_DEBUG "failed to request GPJ1(5) for camera control\n");
	...
	if (err) {
		printk(KERN_DEBUG "failed to request GPJ1(2) for camera control\n");
	...
	if (err) {
		printk(KERN_DEBUG "failed to request GPB6 for camera control\n");
...
static int ce147_power_off(void)
{
	...
	printk(KERN_DEBUG "ce147_power_off by ce147 driver \n");
...
{
	printk(KERN_DEBUG "ce147_power_en by ce147 driver : onoff %d \n", onoff);
...
static int ce147_get_main_sw_fw_version(struct v4l2_subdev *sd)
{
	...
	char main_sw_fw_path[50] = {CE147_FW_F2_SE_PATH};		//S1_KOR froyo [division of camera fw]

	printk(KERN_DEBUG "ce147_get_main_sw_fw_version Enter \n");

#if 1	//S1_KOR froyo [division of camera fw]
	if(state->runmode == CE147_RUNMODE_NOTREADY)
	{
		printk("%s : return because of CE147_RUNMODE_NOTREADY", __func__);
		return -EIO;
	}
	
	if(state->fw.major == 0x31)		// case OP
		memcpy(main_sw_fw_path, CE147_FW_F2_OP_PATH, sizeof(CE147_FW_F2_OP_PATH));

	printk(KERN_DEBUG "%s : path = %s\n", __func__, main_sw_fw_path);
#endif

	if((MAIN_SW_DATE_INFO[0] == 0x00) && (MAIN_SW_DATE_INFO[1] == 0x00) && (MAIN_SW_DATE_INFO[2] == 0x00))
	{
#if 1	//S1_KOR froyo [ division of camera fw ]
		err = ce147_read_fw_bin(main_sw_fw_path, fw_data, &fw_size);
#else
...
#endif
	unsigned char ce147_str_model[10] = "SHW-M110S\0";
#if 0
