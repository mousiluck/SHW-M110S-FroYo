...
{
	return sprintf(buf, "usb access lock : %d\n", usb_access_lock);	
}
...
)
{
	...
	{
		printk("[HERATIC] enable_store: Invalid MicroUSB_I2C\n");
		return -EINVAL;
	}

	if(value != usb_access_lock) {
		if(value == 1) {
			printk("[FSA9480] HERATIC. Set USB Block!!\n");
			// Set Manual switch - Open all port.
			fsa9480_write(fsa9480_i2c_client, REGISTER_MANUALSW1, 0x1);
			mdelay(1);
			// set switching mode to MANUAL
			fsa9480_write(fsa9480_i2c_client, REGISTER_CONTROL, 0x1A);
			msleep(1);
		} else {
			printk("[FSA9480] HERATIC. Set USB Block Release!!\n");
...
static int fsa9480_codec_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err_usb:
		device_destroy((struct class *)switch_dev, MKDEV(0, 0));
