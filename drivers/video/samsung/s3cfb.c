...
#endif
bool window_config_is_enabled(int win_id)
{
	switch(win_id) {
		case 0:
#ifdef CONFIG_FB_S3C_WINDOW0
			return true;
#else
			return false;
#endif
...
static int s3cfb_map_video_memory(struct fb_info *fb)
{
	...
	if (win->owner == DMA_MEM_OTHER)
		return 0;
	if(win->id == CONFIG_FB_S3C_DEFAULT_WINDOW) {
		dev_id = S3C_MDEV_FIMD_FB0 + CONFIG_FB_S3C_DEFAULT_WINDOW;
		fix->smem_start = s3c_get_media_memory_bank(dev_id, 1);
		reserved_size = s3c_get_media_memsize_bank(dev_id, 1);
		fb->screen_base = ioremap_wc(fix->smem_start, reserved_size);
	}
	else {
		if(window_config_is_enabled(win->id)) {
			dev_id = S3C_MDEV_FIMD_FB0 + win->id;
			fix->smem_start = s3c_get_media_memory_bank(dev_id, 1);
			reserved_size = s3c_get_media_memsize_bank(dev_id, 1);
	fb->screen_base = ioremap_wc(fix->smem_start, reserved_size);
		}
		else {
			printk("Oops!!! Please enable CONFIG_FB_S3C_WINDOW%d using make menuconfig\n",win->id);
		}
	}
...
static int s3cfb_set_par(struct fb_info *fb)
{
	...
	// map video memory for non-default window if it not already mapped.
...
static int s3cfb_ioctl(struct fb_info *fb, unsigned int cmd, unsigned long arg)
{
    ...
    case S3CFB_GET_CURR_FB_INFO:
        ...
        next_fb_info.yoffset = s3cfb_get_offset(fbdev,win->id);//var->yoffset;
...
static int s3cfb_init_fbinfo(int id)
{
	...
	var->yres_virtual = CONFIG_FB_S3C_Y_VRES * pdata->nr_buffers[id];
#else
	var->xres_virtual = var->xres;
	var->yres_virtual = var->yres * pdata->nr_buffers[id];
#endif
	var->bits_per_pixel = 32;
	var->xoffset = 0;
	var->yoffset = 0;
...
int s3cfb_register_framebuffer(void)
{
	...
	for (i = 0; i < pdata->nr_wins; i++) {
		...
		// For non-default window
		if(i != pdata->default_win && window_config_is_enabled(i)){
			dev_err(fbdev->dev, "s3cfb_register_framebuffer (%d)\n", i);
