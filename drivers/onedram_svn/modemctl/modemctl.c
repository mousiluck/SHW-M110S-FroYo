...
extern int start_silent_reset;

static unsigned int Sysdown_Reason = 0;
static unsigned int modem_shut_down = 0;
...
struct modemctl_ops {
	...
	void (*modem_shutdown)(struct modemctl *);
};
...
static void msm_shutdown(struct modemctl *);
...
static void msm_boot_on(struct modemctl *mc)
{
	...
	{
		if(mc->gpio_flm_sel_03)
			gpio_set_value(mc->gpio_flm_sel_03, 1);		
	}
}
...
static void msm_boot_off(struct modemctl *mc)
{
	...
	{
		if(mc->gpio_flm_sel_03)
			gpio_set_value(mc->gpio_flm_sel_03, 0);		
	}
}

static void msm_shutdown(struct modemctl *mc)
{
	printk("%s\n", __func__);
	modem_shut_down = 1;
}
...
static int modem_shutdown(struct modemctl *mc)
{
	dev_dbg(mc->dev, "%s\n", __func__);
	if(!mc->ops || !mc->ops->modem_shutdown) {
		// 
		return -ENXIO;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	{
		if(mc->gpio_flm_sel_03)
			p += sprintf(p, "\t%3d %d : FLM sel rev03 \n", mc->gpio_flm_sel_03,
					gpio_get_value(mc->gpio_flm_sel_03));
	}
	if(mc->gpio_sim_ndetect)
...
static void mc_work(struct work_struct *work)
{
	...
	else
	{
		...
		{
			if(cp_boot_ok && !modem_shut_down && !start_silent_reset)
			{
				start_silent_reset=1;
		...
		{
			if(!modem_shut_down && cp_boot_ok)
			{
				printk("cpcrash go to dump 2\n");
				_go_dump();
			}else
			{
				printk("booting,shutdown or reboot, we ignore it \n");
				return;
			}
		}
#else
...
static int __devinit modemctl_probe(struct platform_device *pdev)
{
	...
	{
		mc->gpio_flm_sel_03 = pdata->gpio_flm_sel_03;		
	}
