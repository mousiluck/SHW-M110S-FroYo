...
int set_gpio_dvs(enum perf_level p_lv)
{
	switch(p_lv)
    {
        case L0:
            ...
            break;
        case L1:
		    // 2010.10.08 dukho.kim - set the voltage of 800MHz as 1.25V to solve the problem of lockup during booting time at emul board
		    if(HWREV >= 15)
		    {
			    printk("[Max8998] Set the voltage of 800MHz as 1.25V\n");
			    writel(((readl(S5PV210_GPH0DAT) & ~PMIC_SET_MASK)                                                ), S5PV210_GPH0DAT);	
		    }
		    else
		    {
